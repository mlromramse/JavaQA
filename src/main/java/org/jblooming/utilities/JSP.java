...
import org.jblooming.waf.html.display.FeedbackError;
...
//import org.jblooming.utilities.SmileyUtilities;

/**
 * @author Pietro Polsinelli ppolsinelli@open-lab.com
 * @author Roberto Bicchierai rbicchierai@open-lab.com
 */
public class JSP extends StringUtilities {


  private static Pattern illegalAmpersands = Pattern.compile("(?i)&(?!(nbsp|lt|gt|quote|(?:#\\d{2,6})|amp|);)");
  ...
  public static String w(Period p) {
    if (p == null)
      ...
      return DateUtilities.dateAndHourToString(p.getStartDate()) + "-" + DateUtilities.dateAndHourToString(p.getEndDate());
  }
  ...
  public static void feedbackError(ClientEntry statefulCE, PageContext pageContext) {
    feedbackError(statefulCE, true, pageContext);
  }

  public static void feedbackError(ClientEntry statefulCE, boolean translateError, PageContext pageContext) {
   ...
   *
   * @param tagName  nome del tag (es. "font")
   * @param tagParam parametri da inserire nel tag (es. color="#ff0000" size="3" )
   * @param text     testo
   */
  public static String makeTag(String tagName, String tagParam, String text) {
  ...
  public static String limWr(String content, int maxLength, boolean fromLeft) {
    if (JSP.ex(content)) {
  ...
  /**
   * @param s
   * @return a string in html compatible format tags and apex are all encoded and \n replaced with <br>
   */
  ...
  public static String htmlEncodeAmpersands(String source) {
   // return illegalAmpersands.matcher(s).replaceAll("&amp;");
     if (source != null) {
      String result = StringUtilities.replaceAllNoRegex(source, new String[]{"&"}, new String[]{"&amp;"});
      return result;
    } else
      return null;
  }
  ...
  public static String removeLineFeed(String text) {
    ...
    if (text != null)
      return StringUtilities.replaceAllNoRegex(text, new String[]{"\n", "\f","\r"}, new String[]{" ", " "," "});
    else
      return null;
  }
  ...
  /**
   * codifica il contenuto di str secondo la codifica per le costanti stringa di Javascript
   * da usare per passare valori a script javascript es: onclick="alert('< % =javascriptEncode(variabile) % >')"
   */
  ...
  /**
   * @param what          string to indent
   * @param level         is the level
   * @param pixelForLevel how many pixel for each level
   * @return the string wrapped in a table with the
   */
  public static String indent(String what, int level, int pixelForLevel) {
    return "<table border=\"0\" cellspacing=\"0\" cellpadding=\"0\" width=\"100%\"><tr><td width=\"" + level * pixelForLevel + "\">&nbsp;</td><td>" + what + "</td></tr></table>";
  }

  public static void indent(JspIncluder what, int level, int pixelForLevel, PageContext pageContext) throws IOException, ServletException {
  ...
  public static boolean exForValue(String content, String expectedValue) {
    return JSP.ex(content) && JSP.ex(expectedValue) && content.equals(expectedValue);
  }
